...
## EnumItem<TKey,TGroup>.Group Property

Gets the optional group associated with this enumeration item.

```csharp
public System.Nullable<TGroup> Group { get; set; }
```
...
### Remarks
A value of `null` indicates that the item does not belong to a specific group.
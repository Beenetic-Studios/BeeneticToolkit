#### [Numerics](index.md 'index')
### [BeeneticToolkit.Numerics](index.md#BeeneticToolkit.Numerics 'BeeneticToolkit.Numerics').[RoundingUtils](RoundingUtils.md 'BeeneticToolkit.Numerics.RoundingUtils')

## RoundingUtils.Wrap(decimal, decimal, decimal) Method

Wraps a value within a specified range.

```csharp
public static decimal Wrap(decimal value, decimal min, decimal max);
```
#### Parameters

<a name='BeeneticToolkit.Numerics.RoundingUtils.Wrap(decimal,decimal,decimal).value'></a>

`value` [System.Decimal](https://docs.microsoft.com/en-us/dotnet/api/System.Decimal 'System.Decimal')

The value to wrap.

<a name='BeeneticToolkit.Numerics.RoundingUtils.Wrap(decimal,decimal,decimal).min'></a>

`min` [System.Decimal](https://docs.microsoft.com/en-us/dotnet/api/System.Decimal 'System.Decimal')

The minimum value of the range.

<a name='BeeneticToolkit.Numerics.RoundingUtils.Wrap(decimal,decimal,decimal).max'></a>

`max` [System.Decimal](https://docs.microsoft.com/en-us/dotnet/api/System.Decimal 'System.Decimal')

The maximum value of the range.

#### Returns
[System.Decimal](https://docs.microsoft.com/en-us/dotnet/api/System.Decimal 'System.Decimal')  
The wrapped value.